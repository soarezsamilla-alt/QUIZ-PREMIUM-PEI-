      ...
      <div className="quiz-options flex flex-col gap-3">
        {step.options.map((option, index) => (
          <div
            ...
            className={cn(
              "quiz-option relative flex items-center gap-3.5 p-4 rounded-md cursor-pointer transition-all duration-200 overflow-hidden border-1.5 border-transparent bg-cream group",
              selectedOption === index 
                ? "border-rose bg-rose-pale translate-x-1 shadow-[0_4px_20px_rgba(232,131,154,0.22)]"
                : "hover:border-rose-light hover:translate-x-1 hover:shadow-sm-custom"
            )}
          >
            ...
            <span className="option-icon relative z-10 w-11 h-11 rounded-xl bg-white flex items-center justify-center text-[22px] shadow-[0_2px_8px_rgba(180,120,160,0.12)] transition-transform group-hover:scale-110">
              {option.icon}
            </span>
